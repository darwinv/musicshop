...
elseif(Tools::isSubmit('actualizarcancion')){
		...
		$music = new RoanjaMusic();
		$res = $music->updateMusic((int)Tools::getValue('actualizarcancion'),(int)Tools::getValue('virtualproduct'),Tools::getValue('cancion'),Tools::getValue('autor'),Tools::getValue('genero'));
		$this->clearCache();
	if(!$res){
		$this->_html .= $this->displayError('Could not delete.');
	  }
